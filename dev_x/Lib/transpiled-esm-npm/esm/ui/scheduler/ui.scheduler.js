...
import { createFactoryInstances, disposeFactoryInstances, getTimeZoneCalculator as _getTimeZoneCalculator, getModelProvider, createModelProvider, generateKey } from './instanceFactory';
...
import { getPreparedDataItems } from '../../renovation/ui/scheduler/utils/data'; // STYLE scheduler
...
class Scheduler extends Widget {
  ...
  _updatePreparedItems(items) {
    this.preparedItems = getPreparedDataItems(items, this._dataAccessors, this._getCurrentViewOption('cellDuration'), _getTimeZoneCalculator(this.key));
  }
  ...
  _appointmentsConfig() {
    var config = {
      ...
      onContentReady: () => {
        ...
        (_this$_workSpace = this._workSpace) === null || _this$_workSpace === void 0 ? void 0 : _this$_workSpace.option('allDayExpanded', this._isAllDayExpanded());
      }
    };
    return config;
  }
  ...
  dayHasAppointment(day, rawAppointment, trimTime) {
    var getConvertedToTimeZone = date => {
      return _getTimeZoneCalculator(this.key).createDate(date, {
        path: 'toGrid'
      });
    };

DROP PROCEDURE IF EXISTS sp_vendedor_perfecto_incauca;
DELIMITER //
CREATE PROCEDURE sp_vendedor_perfecto_incauca(IN tmp_almacen INT)
LANGUAGE SQL
DETERMINISTIC
...
BEGIN
	...
	select
		...
		(select count(id) from redenciones where id_afiliado = ven.id_vendedor and temporada = 100) entregas
	from
...
call sp_vendedor_perfecto_incauca(92);
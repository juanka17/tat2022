DROP PROCEDURE IF EXISTS sp_ranking_vendedores_2;
DELIMITER //
CREATE PROCEDURE sp_ranking_vendedores_2()
LANGUAGE SQL
DETERMINISTIC
...
BEGIN
	...
	CREATE TEMPORARY TABLE IF NOT EXISTS t_vendedores AS (
		...
		from 
		   ...
		   left join redenciones red on red.id_afiliado = afi.id and red.temporada = 9
		   left join ciudad ciu on ciu.id = alm.id_ciudad
		where
			ecu.id_periodo in (18,19)
		group by
		   ecu.id_afiliado,

angular.module('estadosRedencionApp', []).controller('estadosRedencionController', function($scope, $http) {
    ...
    $scope.nuevo_estado = { id_operacion: 0, comentario: "", id_usuario: 0 };
    $scope.RegistrarSeguimiento = function() {
        $scope.nuevo_estado.id_redencion = id_redencion;
        $scope.nuevo_estado.id_usuario = $scope.usuario_en_sesion.id;

        var parametros = { catalogo: "seguimiento_redencion", datos: $scope.nuevo_estado };
        $scope.EjecutarLlamado("afiliados", "registrar_seguimiento_redencion", parametros, $scope.ResultadoRegistroSeguimiento);
    ...
    $scope.SumarioFoliosMasivos = function() {
        ...
        angular.forEach(listado, function(folio) {
            ...
            } else if (!$.isNumeric(informacion_folio[0])) { folio_correcto = false; } else if (!$.isNumeric(informacion_folio[1])) { folio_correcto = false; } else if (informacion_folio[2].length > 100) { folio_correcto = false; }
            if (folio_correcto) {

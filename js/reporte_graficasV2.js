angular.module('reporteGraficasApp', ['chart.js']).controller('reporteGraficasController', function ($scope, $http) {
    ...
    // <editor-fold defaultstate="collapsed" desc="Crear Indicadores">
    $scope.CrearFiltros = function ()
    {
        ...
        $scope.EjecutarLlamado("catalogos", "RegistraCatalogoDesdeArrayJSON", parametros, $scope.ObtenerIndicadoresVentas);
    };

    $scope.countChecked = function () {
        $scope.filtro_portafolio = [];
        $scope.filtro_categoria_producto = [];
        $scope.filtro_marcas = [];
        $scope.filtro_sub_marcas = [];
        $scope.filtro_productos = [];
        $scope.filtro_territorios = [];
        $scope.filtro_representantes = [];
        $scope.filtro_distribuidora_madre = [];
        $scope.filtro_distribuidora = [];
        ...
        $(".filtro_portafolio:checked").each(function (index, filtro) {
            $scope.filtro_portafolio.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "portafolio",
                id_filtro: id_filtro
            });
        });
        $(".filtro_categoria_producto:checked").each(function (index, filtro) {
            $scope.filtro_categoria_producto.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "categoria_producto",
                id_filtro: id_filtro
            });
        });
        $(".filtro_marcas:checked").each(function (index, filtro) {
            $scope.filtro_marcas.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "marcas",
        ...
        $(".filtro_sub_marcas:checked").each(function (index, filtro) {
            $scope.filtro_sub_marcas.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "sub_marcas",
                id_filtro: id_filtro
            });
        });
        $(".filtro_productos:checked").each(function (index, filtro) {
            $scope.filtro_productos.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "productos",
                id_filtro: id_filtro
            });
        });
        $(".filtro_territorios:checked").each(function (index, filtro) {
            $scope.filtro_territorios.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "territorios",
                id_filtro: id_filtro
            });
        });
        $(".filtro_representantes:checked").each(function (index, filtro) {
            $scope.filtro_representantes.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "representantes",
                id_filtro: id_filtro
            });
        });
        $(".filtro_distribuidora_madre:checked").each(function (index, filtro) {
            $scope.filtro_distribuidora_madre.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "distribuidora_madre",
                id_filtro: id_filtro
            });
        });
        $(".filtro_distribuidora:checked").each(function (index, filtro) {
            $scope.filtro_distribuidora.push($(filtro).attr("value"));
            id_filtro = $(filtro).attr("value");
            $scope.filtros.push({
                hash_consulta: hash_consulta,
                tipo_filtro: "distribuidora",
                id_filtro: id_filtro
            });
        });
    ...
    $scope.ObtenerIndicadoresVentas = function ()
    {
        ...
        $scope.EjecutarLlamado("reportes", "obtener_indicadores_ventas_checkbox", parametros, $scope.MostrarDatosGrafica);
    ...
    $scope.ObtenerIndicadoresRepresentantes = function ()
    {
        ...
        $scope.EjecutarLlamado("reportes", "obtener_indicadores_representantes_checkbox", parametros, $scope.MostrarDatosGraficaRepresentantes);
    };
    ...
    $scope.ObtenerIndicadoresRankingProductos = function ()
    {
        ...
        $scope.EjecutarLlamado("reportes", "sp_indicadores_ranking_productos_checkbox", parametros, $scope.MostrarIndicadoresRankingProductos);
    };
    ...
    $scope.ObtenerIndicadoresCrecimientoAnual = function ()
    {
        ...
        $scope.EjecutarLlamado("reportes", "obtener_ndicadores_crecimiento_checkbox", parametros, $scope.MostrarDatosGraficaCrecimientoAnual);
    };
    ...
    $scope.MostrarDatosGraficaCrecimientoAnual = function (data)
    {
        ...
        } else
        {
            ...
            let temp_obj_dropsize = {periodo: '', dropsize_2019: 0, dropsize_2020: 0};
            for (i in dg.data)
            {
                ...
                if (temp_obj_dropsize.periodo != dg.data[i].periodo)
                {
                    if (temp_obj_dropsize.periodo != '')
                    {
                        ...
                        temp_obj_dropsize = {periodo: '', dropsize_2019: 0, dropsize_2020: 0};
                    }
            ...
            new Morris.Bar({
                ...
                xkey: 'periodo',
                ykeys: ['dropsize_2019', 'dropsize_2020'],
                labels: ['2019', '2020'],
                barColors: ['#ff0700', '#41C26D', '#1222e4', '#7248B7']
            });
        }
    ...
    $scope.ObtenerIndicadoresCumplimiento = function ()
    {
        ...
        $scope.EjecutarLlamado("reportes", "obtener_indicadores_cumplimiento_checkbox", parametros, $scope.MostrarIndicadoresCumplimiento);
    };
    ...
    $scope.ObtenerIndicadoresCupos = function (id_territorio, id_representante, id_almacen, id_temporada)
    {
        ...
        $scope.EjecutarLlamado("reportes", "sp_indicadores_cupos", parametros, $scope.MostrarIndicadoresCupos);
    };
    ...
    $scope.MostrarIndicadoresCupos = function (data)
    {
        ...
        {
            $scope.ecu_temporada = data;
            //console.log($scope.ecu_temporada);
            let chartData = [];
            let chartResponse = $scope.ecu_temporada.data[0];
